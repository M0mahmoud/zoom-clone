...
const Table = ({ title, desc }: { title: string; desc: string }) => {
  return (
    <div className="flex flex-col items-start gap-2 xl:flex-row">
      ...
      <p className="truncate text-xs font-bold max-sm:max-w-[320px] lg:text-xl">{desc}</p>
    </div>
  );
};
...
const PersonalRoom = () => {
  ...
  const meetingLink = `${process.env.NEXT_PUBLIC_PRODUCTION_URL}meeting/${meetingId}?personal=true`;
